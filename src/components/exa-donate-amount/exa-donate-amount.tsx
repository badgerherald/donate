	...
	private renderAmountGrid() {
		return [
			<ul class="reoccuring">
				<li>
					<a
						class={this.reoccuring == 12 ? "active" : ""}
						onClick={(_) => this.setReoccuring(12)}>
						Monthly
					</a>
				</li>
				<li>
					<a
						class={this.reoccuring == 2 ? "active" : ""}
						onClick={(_) => this.setReoccuring(2)}>
						Each Semester
					</a>
				</li>
				<li>
					<a
						class={this.reoccuring == 0 ? "active" : ""}
						onClick={(_) => this.setReoccuring(0)}>
						One Time
					</a>
				</li>
			</ul>,
			...
			<ul>
				{this.defaults.map((amount) => (
					<li class="suggestion">
						<a
							class={
								this.amount === amount && !this.isCustomAmount ? "current" : ""
							}
							onClick={(_) => this.setAmount(amount, false, true)}>
							${amount}
						</a>
					</li>
				))}
				<li class="suggestion">
					<span
						class={
							(this.amount > 0 && this.isCustomAmount) || this.isTyping
								? "current custom"
								: "custom"
						}>
						<input
					...
					{this.isCustomAmount || this.isTyping ? (
						<button class="button">
